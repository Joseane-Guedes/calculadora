...
for (let i = 0; i < numero.length; i++) {
    numero[i].addEventListener("click", function (e) {
        ...
        let textoNaTela = input.innerHTML;
        let ultimoChar = textoNaTela[textoNaTela.length - 1];
...
for (let i = 0; i < conta.length; i++) {
    conta[i].addEventListener("click", function (e) {
        ...
        if (ultimoChar === "+" || ultimoChar === "-" || ultimoChar === "×" || ultimoChar === "÷") {
            let newString = textoNaTela.substring(0, textoNaTela.length - 1) + e.target.innerHTML;
